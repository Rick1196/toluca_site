 ...
 *
 *
 * @method     ChildMigrationsQuery orderById($order = Criteria::ASC) Order by the id column
 ...
 *
 * @method     ChildMigrationsQuery groupById() Group by the id column
 ...
 *
 * @method     ChildMigrations findOneById(int $id) Return the first ChildMigrations filtered by the id column
 ...
 *
 * @method     ChildMigrations requireOneById(int $id) Return the first ChildMigrations filtered by the id column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 * @method     ChildMigrations requireOneByMigration(string $migration) Return the first ChildMigrations filtered by the migration column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 * @method     ChildMigrations requireOneByBatch(int $batch) Return the first ChildMigrations filtered by the batch column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 *
 ...
 * @method     ChildMigrations[]|ObjectCollection findById(int $id) Return ChildMigrations objects filtered by the id column
...
abstract class MigrationsQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'blogrodolfodb', $modelName = '\\Migrations', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT id, migration, batch FROM migrations WHERE id = :p0';
        try {
    ...
    public function filterByPrimaryKey($key)
    {
        ...
        return $this->addUsingAlias(MigrationsTableMap::COL_ID, $key, Criteria::EQUAL);
    }
    ...
    public function filterByPrimaryKeys($keys)
    {
        ...
        return $this->addUsingAlias(MigrationsTableMap::COL_ID, $keys, Criteria::IN);
    }
    ...
    public function filterById($id = null, $comparison = null)
    {
        if (is_array($id)) {
            $useMinMax = false;
            if (isset($id['min'])) {
                $this->addUsingAlias(MigrationsTableMap::COL_ID, $id['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($id['max'])) {
                $this->addUsingAlias(MigrationsTableMap::COL_ID, $id['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(MigrationsTableMap::COL_ID, $id, $comparison);
    }
     ...
     * $query->filterByMigration('%fooValue%', Criteria::LIKE); // WHERE migration LIKE '%fooValue%'
     * </code>
     *
    ...
    {
        if ($migrations) {
            $this->addUsingAlias(MigrationsTableMap::COL_ID, $migrations->getId(), Criteria::NOT_EQUAL);
        }
